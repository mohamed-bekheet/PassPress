...
def create_models(spec: LegacyModelSpec, generator_name: str) -> int:
    ...
    pigs = make_pigs(spec.spec)

    CASE_THT_TYPE = "tht"
    CASE_SMD_TYPE = "smd"
    if spec.spec["type"] == CASE_THT_TYPE:
        pins = make_pins_tht(spec.spec)

    if spec.spec["type"] == CASE_SMD_TYPE:
