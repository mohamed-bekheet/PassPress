...
def create_models(spec: LegacyModelSpec, generator_name: str) -> int:
    ...
    case = make_case(spec.spec)
    case_top = make_case_top(spec.spec)

    if spec.spec["pintype"] == CASE_THT_TYPE:
        pins = make_pins_tht(spec.spec)

    if spec.spec["pintype"] == CASE_THT_N_TYPE:
        pins = make_pins_tht_n(spec.spec)

    if spec.spec["pintype"] == CASE_SMD_TYPE:
