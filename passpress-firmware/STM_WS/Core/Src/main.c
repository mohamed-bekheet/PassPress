...
#define HID_CTRL_CMD_SET_MODE      0x03U
#define HID_CTRL_CMD_GET_BUTTON    0x04U
#define HID_CTRL_CMD_SET_BUTTON    0x05U
#define HID_CTRL_CMD_ENTER_BOOTLOADER 0x06U
#define HID_STATUS_TYPE_PING_ACK   0x81U
#define HID_STATUS_TYPE_SNAPSHOT   0x82U
#define HID_STATUS_TYPE_BUTTON_CFG  0x83U
#endif
...
static void HID_UpdateButtonConfigReport(uint8_t seq, uint8_t buttonIdx);
#endif
...
static void HID_UpdateButtonConfigReport(uint8_t seq, uint8_t buttonIdx)
{
  uint8_t password[TOUCH_PASSWORD_MAX_LEN + 1U] = {0U};
  uint8_t passwordLen = 0U;
  uint8_t appendEnter = 0U;
  uint8_t report[HID_FEATURE_REPORT_SIZE] = {0U};

  if ((buttonIdx >= TOUCH_BUTTON_COUNT) ||
      (TouchPassword_GetButtonConfig(buttonIdx, password, &passwordLen, &appendEnter, (uint8_t)sizeof(password)) == 0U))
  {
    HID_UpdateStatusReport(0xFFU, seq);
    return;
  }

  report[0] = HID_REPORT_ID_STATUS_IN;
  report[1] = HID_STATUS_TYPE_BUTTON_CFG;
  report[2] = seq;
  report[3] = buttonIdx;
  report[4] = appendEnter;
  report[5] = passwordLen;

  if (passwordLen > 0U)
  {
    memcpy(&report[6], password, passwordLen);
  }
...
static void HID_ProcessControlReport(void)
{
  ...
  switch (report[1])
  {
    ...
    case HID_CTRL_CMD_SET_MODE:
      if ((length >= 4U) && (TouchPassword_SetMode(report[3]) != 0U))
      {
        HID_UpdateStatusReport(HID_STATUS_TYPE_SNAPSHOT, report[2]);
      }
      else
    ...
    case HID_CTRL_CMD_GET_BUTTON:
      if ((TouchPassword_GetMode() == TOUCH_MODE_ADMIN) && (length >= 4U))
      {
        HID_UpdateButtonConfigReport(report[2], report[3]);
      }
      else
      {
        HID_UpdateStatusReport(0xFFU, report[2]);
    ...
    case HID_CTRL_CMD_SET_BUTTON:
      if ((TouchPassword_GetMode() == TOUCH_MODE_ADMIN) && (length >= 6U))
      {
        uint8_t buttonIdx = report[3];
        uint8_t appendEnter = report[4];
        uint8_t passwordLen = report[5];

        if ((buttonIdx < TOUCH_BUTTON_COUNT) &&
            (passwordLen <= TOUCH_PASSWORD_MAX_LEN) &&
            (length >= (uint16_t)(6U + passwordLen)) &&
            (TouchPassword_SetButtonConfig(buttonIdx, &report[6], passwordLen, appendEnter) != 0U) &&
            (TouchPassword_SaveConfig() != 0U))
        {
          HID_UpdateButtonConfigReport(report[2], buttonIdx);
        }
        else
        {
          HID_UpdateStatusReport(0xFFU, report[2]);
    ...
    case HID_CTRL_CMD_ENTER_BOOTLOADER:
      /* Only allow entering bootloader from ADMIN mode */
      if ((TouchPassword_GetMode() == TOUCH_MODE_ADMIN) && (TouchPassword_EnterBootloader() != 0U))
      {
        /* If function returns, something went wrong - report error */
        HID_UpdateStatusReport(0xFFU, report[2]);
      }
      /* On success the device will reset, no response needed */
      break;
