...
MODE_COLORS = {
    DeviceMode.NON_TRUSTED: "#94A3B8",
    DeviceMode.TRUSTED: "#3B82F6",
    DeviceMode.ADMIN: "#EAB308",
}
...
class BoardMapWidget(QWidget):
    ...
    def _draw_rect(self) -> QRectF:
        ...
        scaled = self._pixmap.scaled(
            board_rect.size().toSize(),
            Qt.KeepAspectRatio,
            Qt.SmoothTransformation,
        )
        return QRectF(
            board_rect.center().x() - scaled.width() / 2,
            board_rect.center().y() - scaled.height() / 2,
            scaled.width(),
            scaled.height(),
        )
    ...
    def paintEvent(self, event) -> None:
        ...
        if self._pixmap.isNull():
            ...
            scaled = self._pixmap.scaled(board_rect.size().toSize(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
            painter.save()
            painter.translate(board_rect.center())
            painter.rotate(self._rotation_deg)
            painter.translate(-board_rect.center())
            painter.drawPixmap(board_rect.toRect(), scaled)
            painter.restore()
...
class MainWindow(QMainWindow):
    ...
    def _request_admin(self) -> None:
        ...
        if not self._transport.verify_admin(dialog.password):
            QMessageBox.warning(self, "Admin", "Admin verification failed.")
    ...
    def _on_status_updated(self, status: DeviceStatus) -> None:
        ...
        self.mode_badge.setStyleSheet(
            f"background-color: {color}26; border: 1px solid {color}; border-radius: 10px; padding: 7px 10px; font-weight: 650;"
        )
